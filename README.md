...
- Matching star labels link quantities to their PO in Remarks: the first PO in each trip has no label, the second uses `*`, the third uses `**`, and further POs add one star each. Labels restart for each trip and contain no numbers. Shared quantities list contributing nonempty labels below the total; quantity cells remain numeric for calculations.
