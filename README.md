...
Choose **Daily dispatch** in the sidebar to upload documents, or **Review & print** to return to your results. Small-tag pages default to one copy each. Each small tag represents one rack (repeated part tags count separately). The large page is identified by its ORDER NUMBER and SKID NO fields and matched by destination and delivery sequence. Its copies equal two per rack, except Bukit Raja `614` and Shah Alam `HU83`, `238X`, `HU82`, which need one per rack. Three HOOK racks therefore need three large-page copies. The supplied six-rack example produces two original small-tag sheets plus twelve large-page copies: fourteen printed pages.
