...
- Matching star labels link quantities to their PO in Remarks: no label for a trip with one PO, or `*`, `**`, and further repeated stars for multiple POs. Labels restart for each trip and contain no numbers. Shared quantities list all contributing labels below the total; quantity cells remain numeric for calculations.
